...
| Hidden parameterized benchmark v2 | partial | Controller-only generation derives all range names from the seed. Apply attests API-returned UIDs; cleanup uses UID preconditions, detects replacements, and waits for residue removal. `ScoreWithCluster` checks live actor, target, and control state. A committed private/public seed inventory and a clean full Kind matrix exit are absent. | Contract tests cover strict decoding, tampering prerequisites, canonical hashing, projection binding, seed derivation, paired determinism, resource rendering, public/private separation, exact and live-state scoring, and lifecycle identity/residue checks. The expanded Kind matrix currently hangs after cleanup and is not passing evidence. | `7feaaf8` through current worktree |
