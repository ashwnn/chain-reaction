...
| Hidden parameterized benchmark v2 | partial | Controller-only contract generation covers eight archetype shapes and paired controls; Kubernetes rendering, hidden range storage, lifecycle, and executable scoring are absent. | Contract tests cover strict decoding, tampering prerequisites, canonical hashing, projection binding, seed derivation, and paired determinism. | pending local commit |
...
| CR-260 | Controller-only contract generator now covers eight archetype shapes; no Kubernetes renderer or public commitment inventory exists. | in progress |
