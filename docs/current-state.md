...
| Hidden parameterized benchmark v2 | partial | Controller-only generation, rendering, private-seed writes, public commitments, and exact evidence scoring are implemented; live-cluster lifecycle and scorer inputs are absent. | Contract tests cover strict decoding, tampering prerequisites, canonical hashing, projection binding, seed derivation, paired determinism, rendering, public/private separation, and exact scoring. | pending local commit |
